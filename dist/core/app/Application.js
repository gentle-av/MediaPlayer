...
export class Application {
    ...
    render() {
        ...
        if (this.mainContainer) {
            ...
            console.log('✅ [Application] Разметка MainFrame успешно добавлена в DOM-дерево.');
        }
        else {
            console.error('❌ [Application] Критическая ошибка: Контейнер #main не обнаружен на странице!');
        }
    }
    async initialize() {
        console.log('⏳ [Application] Ожидание загрузки данных с серверов...');
        await this.mainFrame.initialize();
        console.log('📦 [Application] Данные MainFrame загружены. Запуск InitialPlaybackSyncService...');
        await this.initialPlaybackSyncService.syncPlaybackState();
        console.log('✨ [Application] Асинхронная синхронизация плейбека завершена.');
    }
}
