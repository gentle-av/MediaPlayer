...
export class RefreshManagerModal {
  ...
  private async startScanningProcess(): Promise<void> {
  ...
  private pollProgress(
    ...
    const intervalId = window.setInterval(async () => {
      fakeProgress += 8;
      if (fakeProgress > 90) fakeProgress = 95;
  ...
  private async showStatistics(stats: any): Promise<void> {
    const bodyContainer = document.getElementById('refreshManagerModalBody');
    if (!bodyContainer) return;
    bodyContainer.innerHTML = '';
    ...
    if (actionsContainer) {
      ...
      closeBtn.addEventListener('click', () => {
        ...
        ToastService.getInstance().show(
          'Медиатека успешно синхронизирована',
          'success',
        );
      });
